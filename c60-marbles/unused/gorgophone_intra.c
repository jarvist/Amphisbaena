...
#define TOFS 3 //number of ToF's to run to build up stable statistics
...
gorgophone()
{
   ...
   for (BIAS=0.09;BIAS<=0.1;BIAS+=0.04)
     {
...
//	    fprintf(stderr,"Calculating hop to: (x,y,z) %d %d %d (dx,dy,dz) %d %d %d\n",x+dx,y+dy,z+dz,dx,dy,dz);
	    ...
	    if (lattice[x+dx][y+dy][z+dz]==lattice[x][y][z]) //hops to self are order of mag faster
	      rate*=100.0; 
