...
#include "amphisbaena_toast.h" //simulation parameters & prototypes
...
//	    lattice_energy();
	...
	{
	 perc_describe(); //number perc lattice with unit of snake
...
//	print_lattice_pnm_file((int)(beta*1000.00));
    ...
    save_lattice_file("lattice.dat");
   print_povray("snakes.pov");
    fprintf(stderr,"Saved a version of lattice.dat & snakes.pov.\n");
...
{ 
 ...
 for (s=0;s<num_snakes;s++)
 {
	 //set head + tail bold for display, using perc lattice
       perc [snakes[s].oroborus[snakes[s].head].x]
            [snakes[s].oroborus[snakes[s].head].y]
            [snakes[s].oroborus[snakes[s].head].z] = 1;

       perc [snakes[s].oroborus[(snakes[s].head-1)%snakes[s].segs].x]
...
// ENERGY CALCULATION FOR MONTE CARLO STEPS
	   ...
	   int length, int segment)
{
  ...
  int order[4] = { 1, 2, 3, 4 }; //order we do the directions in 
