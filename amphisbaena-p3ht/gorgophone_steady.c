...
#define MAX_HOPPERS 20000 //00 //number of hoppers simultaneously ToF'ing
...
#define TOF_TIME 18000.0 //time at which to end ToF - c.f. RATE above
#define TOFS 1 //number of ToF's to run to build up stable statistics

#define ntmax 81 //150 //number of time bins (always geometric)
...
int main(int argc, char *argv[])
{
   ...
   if (argc!=2)
   {
	   fprintf(stderr,"Bork Bork Bork. One argument please: %s lattice.dat\n",argv[0]);
   	return(-1);
   }
...
*/
   DIRECTION='Z';
   sprintf(name,"%s_H20000.ssc",argv[1]);
   fo=fopen(name,"w");
   gorgophone();
	fclose(fo);
/*
...
gorgophone()
{
   ...
   for (BIAS=0.01;BIAS<=0.09;BIAS+=0.04) //BIAS is a global variable
     {
...
//   if (DIRECTION=='Y')
//	   BIAS*=4.0/15.0; //scale BIAS for different lattice length in Y: pi-pi stacking in P3HT
...
/*	if ( (DIRECTION=='Z'&&hoppers[fastesthopper].loc.z<4) 
			|| (DIRECTION=='Y'&&hoppers[fastesthopper].loc.y<4)
			|| (DIRECTION=='X'&&hoppers[fastesthopper].loc.x<4) ) //if we've reached the exit electrode
	  {     
...
fprintf(stderr,"\n");   
	...
	fprintf(fo,"#Tof Sim %d Complete %d tof_count\n",sims+1,tof_count);
/*     for (i=0;i<ntmax;i++)
     {
...
//   print_waits();
  ...
  for (i=0;i<ntmax;i++)
	  fprintf(fo,"%f %f %f\n",BIAS,1000.0*(float)i,(float)currentevents[i]/(1000.0*TOFS*MAX_HOPPERS));
...
/*   for (i=0;i<ntmax;i++)
  ...
  */ 
//   if (DIRECTION=='Y')                                                                                BIAS/=4.0/15.0; //scale BIAS for different lattice length in Y: pi-pi stacking in P3HT
   //change BIAS back so as not to break for loop
   //
...
//   nt=(int)(log(simtime)/log(timefac));
   ...
   nt=(int)(simtime/1000.0);
   
   //   fprintf(stderr,"Simtime: %f\tnt: %d\n",simtime,nt);
   if (nt<0) nt=0;

   if (DIRECTION=='Z')
   if ( hoppers[hopper].loc.z-hoppers[hopper].dst.z < Z/2 &&  hoppers[hopper].loc.z-hoppers[hopper].dst.z > -Z/2) //ignore current from jumping across board
	   currentevents[nt]+=hoppers[hopper].loc.z-hoppers[hopper].dst.z;
   
   if (DIRECTION=='Y')
	   currentevents[nt]+=hoppers[hopper].loc.y-hoppers[hopper].dst.y;

   if (DIRECTION=='X')
	   currentevents[nt]+=hoppers[hopper].loc.x-hoppers[hopper].dst.x;
...
/*	if (DIRECTION=='Z')
		z=Z-1-rand_int(Z); //even distribution of charge carriers in first 1/10th of material
...
//	    fprintf(stderr,"\nTable %d %d %d\t",x,y,z);
   ...
   for (j=0;j<sphere_bits;j++) // ~32 loops for 5x5x5 sphere
	 ...
	 {
	    dx=sphere[j].x; dy=sphere[j].y; dz=sphere[j].z; pbcdz=sphere[j].z;

                if ((z+dz)>=Z) dz-=Z; // PBCs in Z direction !!! 
	 	if ( (z+dz)<0 ) dz+=Z;

//		if (dz>Z/2 || dz<-Z/2) fprintf(stderr,"Hop across PBC: (x,y,z) %d %d %d with (dx,dy,dz) %d %d %d\n",x,y,z,dx,dy,dz);
//PBC ADDITION CODE - JMF 13-11-07
...
//	    fprintf(stderr,"Calculating hop to: (x,y,z) %d %d %d (dx,dy,dz) %d %d %d\n",x+dx,y+dy,z+dz,dx,dy,dz);
	    ...
	    if (DIRECTION=='Z')
	       dE-=(double)pbcdz*BIAS; //energy required to move
	    if (DIRECTION=='X')
		    dE-=(double)dx*BIAS;
	    if (DIRECTION=='Y')
		    dE-=(double)dy*BIAS;
	    ...
	    rate=RATE 
	      ...
	      * exp( - ( sqrt((double)((dx*dx)+(dy*dy)+(pbcdz*pbcdz)))*(double)IR_0) );
...
//   fprintf(stderr,"Chosen rate: %f\t",chosen_rate);
   ...
   if ((z+sphere[i].z)>=Z) dz=-Z; // PBC Z offset 
   if ( (z+sphere[i].z)<0 ) dz=Z;

   hoppers[hopper].dst.x=x+sphere[i].x;
   hoppers[hopper].dst.y=y+sphere[i].y;
   hoppers[hopper].dst.z=z+sphere[i].z+dz; //PBC shift
...
//if (dz!=0) fprintf(stderr,"PBC: from %d %d %d to %d %d %d\n",x,y,z,hoppers[hopper].dst.x,hoppers[hopper].dst.y,hoppers[hopper].dst.z);
