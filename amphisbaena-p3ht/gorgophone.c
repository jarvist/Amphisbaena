...
#define TOFS 0 //number of ToF's to run to build up stable statistics
...
*/
   DIRECTION='X';
   sprintf(name,"%s_smooth.tof",argv[1]);
   fo=fopen(name,"w");
   gorgophone();
	fclose(fo);
/*
...
gorgophone()
{
   ...
   for (BIAS=0.01;BIAS<=0.01;BIAS+=0.04) //BIAS is a global variable
     {
...
//	    fprintf(stderr,"\nTable %d %d %d\t",x,y,z);
	...
	ratetables[x][y][z][sphere_bits]=0.0; //used to store 'total rate'
       	...
       	if (lattice[x][y][z]<0) continue; //don't calc for empty sites
...
//	    fprintf(stderr,"Calculating hop to: (x,y,z) %d %d %d (dx,dy,dz) %d %d %d\n",x+dx,y+dy,z+dz,dx,dy,dz);
	    ...
	    if (DIRECTION=='Z')
	       dE-=(double)dz*BIAS; //energy required to move
	    if (DIRECTION=='X')
		    dE-=(double)dx*BIAS;
	    if (DIRECTION=='Y')
		    dE-=(double)dy*BIAS;
	    ...
	    if (lattice[x+dx][y+dy][z+dz]==lattice[x][y][z]) //if hopping to another bit of the same snake
	      ...
	      printf("%d %d\n",lattice[x+dx][y+dy][z+dz],lattice[x][y][z]);//print graph edges for inter-snake connections
